...
export function getComparisonVerdict(
    ...
    // Price-focused summary (Amazon Compliance: no fixed prices in text)
    if (priceDiffPercent > 15) {
        ...
        if (winner === cheaper) {
            summary = `${winnerName} oferece performance superior por um preço mais acessível. O investimento adicional no ${loserName} não se justifica pelas funcionalidades extras.`;
            keyDifference = `${winnerName} é mais acessível e ainda assim superior`;
        } else {
            ...
            keyDifference = `${winnerName} justifica o investimento adicional`;
        }
    }
